...
5. Без GPU-worker /v1/capabilities показывает training_available=false. Обучение проверяется
   отдельно по docs/TRAINING.md; новые запросы требуют dataset_id и base_model.
